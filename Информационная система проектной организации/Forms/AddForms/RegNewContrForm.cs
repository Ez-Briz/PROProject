        ...
        private void RegisterNewContr()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    string cmdStr = "INSERT INTO Contract([Name], [Description], [ManagerFIO], [ProjCount]) VALUES" +
                        "(@Name, @Desc, @FIO, @Count)";
                    string checkValueCmd = "SELECT [ID] FROM [Contract]" +
                        "WHERE @Name = [Name]";
